...
# 二次元化
def dim2(W1):	# W1=三次元データ
    W2=np.zeros(shape=[9,9],dtype='int')
    for i in range(9):
        for j in range(9):
            for k in range(9):
                if W1[i,j,k]==0:
...
# マスク作成
def msk1(W1):			# W1:True確定ノード三次元データ
   				# W2:マスクノード三次元データ
    W2=np.zeros(shape=[9,9,9],dtype='int')
...
# W1のマスク計算してSUN(単独不確定ノード)が見つかったらW2に１を入れる
def msk2(W1):
    ext=0
    W2=np.zeros(shape=[9,9,9],dtype='int')
# 行、列ごとにカラム方向のマスク和を計算
    ...
    for bi1 in range(3):
        for bj1 in range(3):
            for k in range(9):
                ...
                for bi2 in range(3):
                    for bj2 in range(3):
                        sum=sum+W1[3*bi1+bi2,3*bj1+bj2,k]
                if sum==8:
                    for bi3 in range(3):
                        for bj3 in range(3):
                            num=W1[3*bi1+bi3,3*bj1+bj3,k]
                            if num==0:
                                ext=1
                                W2[3*bi1+bi3,3*bj1+bj3,k]=1
...
# マスクのマージ
def merge(W1,W2):		# W1とW2をマージしてW3へ
    W3=np.zeros(shape=[9,9,9],dtype='int')
    for i in range(9):
        for j in range(9):
            for k in range(9):
                num1=W1[i,j,k]
...
def IndMask(W1):	# W1=CubeMask
    ...
    ext=0
    for bi1 in range(3):		# i1=ブロック行
        for bj1 in range(3):	# j1=ブロック列
            for k in range(9):
            # ブロック内の未確定ノード数カウント
                bnum=0
                for bi2 in range(3):
                    for bj2 in range(3):
                        num=W1[3*bi1+bi2,3*bj1+bj2,k]
                        if num==0:
                            bnum=bnum+1
            ...
            # ブロック行と行の比較
                ...
                for bi2 in range(3):
                    bnumi=0
                    for bj2 in range(3):	# ブロック行内の未確定ノード数カウント
                        num=W1[3*bi1+bi2,3*bj1+bj2,k]
                        if num==0:
                            bnumi=bnumi+1
                        if bnumi>1:
                            ...
                            if bnumi==numi:	# ブロックにマスク
                                if bnumi==bnum:
                                    ...
                                    for bi3 in range(3):
                                        for bj3 in range(3): 
                                            if bi3==bi2:
                                                W3[3*bi1+bi3,3*bj1+bj3,k]=1	# core nodes
                                                #pass
                                            else:
                                                W2[3*bi1+bi3,3*bj1+bj3,k]=1	# mask nodes
                                                ext=1
                            if bnumi==bnum:	# 行にマスク
                                if bnumi==numi:
                                    pass
                                    ...
                                    for r in range(9):
                                        if r==3*bj1 or r==3*bj1+1 or r==3*bj1+2:
                                            W3[3*bi1+bi2,r,k]=1	# core nodes
                                            #pass
                                        else:
                                            W2[3*bi1+bi2,r,k]=1	# mask nodes
                                            ext=1
           ...
           # ブロック列と列の比較
                ...
                for bj2 in range(3):
                    bnumj=0
                    for bi2 in range(3):	# ブロック内の未確定ノード数カウント
                        num=W1[3*bi1+bi2,3*bj1+bj2,k]
                        if num==0:
                            bnumj=bnumj+1
                        if bnumj>1:
                ...
                # 比較
                            ...
                            if bnumj==bnum:	# 同じ列にマスク
                                if bnumj==numj:
                                    ...
                                    for l in range(9):
                                        if l==3*bi1 or l==3*bi1+1 or l==3*bi1+2:
                                            W3[l,3*bj1+bj2,k]=1	# core nodes
                                            #pass
                                        else:
                                            W2[l,3*bj1+bj2,k]=1	# mask nodes
                                            ext=1
...
def s_web_main(level,QX):
    ...
    CubeMaskTemp0=np.zeros(shape=[9,9,9],dtype='int')
    ...
    while x1<81:
        ...
        CubeMaskTemp0=msk1(CubeTemp0)

        # 単独不確定ノードのマスクとCubeMaskTempをマージ

        Wsave1=merge(CubeMaskTemp,CubeMaskTemp0)
        CubeMaskTemp=cube_copy(Wsave1)

        # 元のマスクと一時マスクをマージ
        
        Wsave1=merge(CubeMask,CubeMaskTemp)
        CubeMask=cube_copy(Wsave1)

        # 初期確定ノードと一時確定ノードをマージ

        Wsave2=merge(CubeTemp,CubeTemp0)
        CubeTemp=cube_copy(Wsave2)

        # マスク計算して新たな不確定ノードをセット
        CubeTemp0=np.full(shape=[9,9,9],fill_value=int(0),dtype='int')
        
        x2,Wsave3=msk2(CubeMask)	# SUN exists when x2=1
        CubeTemp0=cube_copy(Wsave3)

        Wsave4=merge(CubeTemp,CubeTemp0)
        CubeTemp=cube_copy(Wsave4)
        Qtemp0=np.full(shape=[9,9],fill_value=int(0),dtype='int')
        ...
        # 回答完了(x1=81)なら終了ルーチンへ(while loop condition)
                                ...
                                if x4==1:					# N国同盟（列）のマスクが追加されている
                                    ...
                                    if x4==1:					# 隠れN国同盟（行）のマスクが追加されている
                                        ...
                                        if x4==1:# N国同盟（行）のマスクが追加されている
                                            ...
                                            WX=np.zeros(shape=[9,9,9],dtype='int')
                                            WY=np.zeros(shape=[9,9,9],dtype='int')
                                            WX=c_to_b(CubeMask)			# ブロック面を列面に変換
                                            mode="ALI_HB"
    ...
    return x1,result_level,message,Qtemp
